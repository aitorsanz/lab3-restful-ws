...
public class AddressBookServiceTest {
	...
	public void serviceIsAlive() throws IOException {
		...
		//Testing the operation GET/contacts

		Response idempotentResponse = client.target("http://localhost:8282/contacts")
				.request().get();
		assertEquals(response.getStatus(), idempotentResponse.getStatus());
		assertEquals(0, idempotentResponse.readEntity(AddressBook.class).getPersonList()
				.size());
	}
	...
	public void createUser() throws IOException {
		...
		assertEquals(201, idempotentResponse.getStatus());
		assertEquals(juanURI, idempotentResponse.getLocation());
		assertEquals(MediaType.APPLICATION_JSON_TYPE, response.getMediaType());
		juanUpdated = idempotentResponse.readEntity(Person.class);
		assertEquals(juan.getName(), juanUpdated.getName());
		assertEquals(1, juanUpdated.getId());
		assertEquals(juanURI, juanUpdated.getHref());
	...
	public void createUsers() throws IOException {
		...
		//Testing operation GET /contacts/person/3
		Response idempotentResponse = client.target("http://localhost:8282/contacts/person/3")
				.request(MediaType.APPLICATION_JSON).get();
		assertEquals(response.getStatus(), idempotentResponse.getStatus());
		assertEquals(MediaType.APPLICATION_JSON_TYPE, response.getMediaType());
		mariaUpdated = idempotentResponse.readEntity(Person.class);
		assertEquals(maria.getName(), mariaUpdated.getName());
		assertEquals(3, mariaUpdated.getId());
		assertEquals(mariaURI, mariaUpdated.getHref());
	}
	...
	public void listUsers() throws IOException {
		...
		//Testing operation GET
		Response idempotentResponse = client.target("http://localhost:8282/contacts")
				.request(MediaType.APPLICATION_JSON).get();
		assertEquals(response.getStatus(), idempotentResponse.getStatus());
		assertEquals(MediaType.APPLICATION_JSON_TYPE, idempotentResponse.getMediaType());
		addressBookRetrieved = idempotentResponse
				.readEntity(AddressBook.class);
		assertEquals(2, addressBookRetrieved.getPersonList().size());
		assertEquals(juan.getName(), addressBookRetrieved.getPersonList()
				.get(1).getName());
	...
	public void deleteUsers() throws IOException {
		...
		//Testing operation DELETE
		Response idempotentResponse = client.target("http://localhost:8282/contacts/person/2")
				.request().delete();
		assertEquals(response.getStatus(), idempotentResponse.getStatus());
	}
